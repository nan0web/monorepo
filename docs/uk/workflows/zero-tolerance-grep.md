...
ABSOLUTE BAN: The AI agent MUST NOT use the `grep_search` tool without `Includes` bounds. ALWAYS restrict `grep_search` with glob patterns (`Includes: ['*.js', '!**/node_modules/*', '!**/.git/*']`). NEVER search blindly across the entire monorepo or packages/ directory as it causes timeouts.
