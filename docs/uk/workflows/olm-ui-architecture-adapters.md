...
Згідно з цією архітектурою, **Доменна Модель** є абсолютним, агностичним центром. Вона нічого не знає про середовище, у якому працює. 
Усе інше — це **Адаптери**:
1. **UI Адаптери** (`ui-react`, `ui-cli`, `ui-chat`) — це адаптери для взаємодії з користувачем.
2. **Зовнішні Адаптери** (наприклад, Whisper для розпізнавання голосу чи Stripe для оплат) — це адаптери для зовнішніх сервісів.
3. **DB Адаптери** (`db-fs`, `db-browser`) — це адаптери для збереження даних. Сама `db` є просто абстракцією (опцією/середовищем), доступною для Моделі, тоді як реальна її імплементація інжектується ззовні. У структурі додатку **не існує папки `src/db/`**, адже база даних підключається з глобального середовища пакетів `@nan0web`.
