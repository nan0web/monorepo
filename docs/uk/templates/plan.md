...
> **Мета:** Короткий і ясний опис суті та кінцевого результату.
...
## 📋 2. Етапи та Задачі (Milestones & Tasks)
...
## 🧪 3. Загальні Критерії Прийомки (Definition of Done)
- [ ] **Контракти (TDD):** Всі `*.spec.js` зелені (100% Pass).
- [ ] **Регресія:** Контракти закриваються через `@nan0web/release close` (`spec.js` -> `test.js`).
- [ ] **Data-First:** Немає `fs`, використовується `@nan0web/db`.
- [ ] **Git Cleanliness:** Відсутність незв'язаних змін у `git status`.

---

## 📝 4. Журнал Зворотного Зв'язку (Feedback & Decisions Log)
> *Append-only: фіксуємо зауваження Архітектора, помилки та рішення.*
