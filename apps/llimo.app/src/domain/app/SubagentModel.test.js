...
describe('SubagentModel', () => {
	...
	describe('OLMUI Generator contract', () => {
		it('run() is an async generator', () => {
			...
			const m = new SubagentModel({ model: 'test', input: 'test' }, { ai: mockAI, modelInfo: { id: 'test', provider: 'test' } })
			const gen = m.run()
		...
		it('yields error log when no model and no strategy', async () => {
			...
			assert.equal(first.value.type, 'show')
		...
		it('yields error log when no input and no file', async () => {
			const m = new SubagentModel({ model: 'qwen' }, { ai: {}, modelInfo: { id: 'qwen', provider: 'test' } })
			const gen = m.run()
		...
		it('yields status intent with model and provider', async () => {
			...
			const m = new SubagentModel({ model: 'qwen', input: 'hello' }, { ai: mockAI, modelInfo: { id: 'qwen', provider: 'openrouter' } })
			const gen = m.run()
		...
		it('full happy path: status → progress → chunk → summary → result', async () => {
			...
			const mockAI = /** @type {any} */ ({ streamText: () => mockResult })
			const m = new SubagentModel({ model: 'qwen', input: 'hello' }, { ai: mockAI, modelInfo: { id: 'qwen', provider: 'openrouter', pricing: { calc: () => 0.001 } } })
			const gen = m.run()
