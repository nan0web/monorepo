...
export class WebShopperModel extends AiModelAsApp {
	...
	constructor(data = {}, options = {}) {
		super(data, options)
		/** @type {string} */ this.query = data.query || ''
	...
	async *run() {
		...
		if (!this.query) {
			yield show(t(WebShopperModel.UI.errorMissingQuery), 'error')
			return result({ success: false })
		}

		yield progress(t(WebShopperModel.UI.progressAnalysis, { $query: this.query }))

		const ai = injectedAi || new AI({ strategy: new AI.Strategy({ level: 'smart' }) })
...
В результатах SEARCH_WEB ти побачиш посилання на сторінки загальних категорій. Сніпетах часто пише "у наявності 3000 товарів", "ціна від 100 грн". Це НЕ конкретний товар!
...
`
		...
		for (let step = 0; step < 15; step++) {
			let text = ''
			yield progress(t(WebShopperModel.UI.progressThinking, { $step: String(step + 1) }))
			...
			try {
				...
				// Fallbacks handled internally
			}

			if (!text) {
				yield show(t(WebShopperModel.UI.errorModelsFailed), 'error')
				return result({ success: false })
			}
			...
			if (toolCall && toolCall.tool === 'SEARCH_WEB') {
				...
				yield progress(t(WebShopperModel.UI.progressSearching, { $query: toolCall.query }))
				...
				yield progress(t(WebShopperModel.UI.progressReading, { $url: toolCall.url }))
				...
				yield show(t(WebShopperModel.UI.warnHallucination), 'warn')
				continue
			} else {
		...
		yield show(t(WebShopperModel.UI.errorLoopLimit), 'warn')
		return result({ success: false })
	}
}
