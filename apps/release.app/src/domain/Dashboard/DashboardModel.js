...
import { RegistryModel } from '../Registry/index.js'
import {
	parseFrontmatter,
...
} from '../../../../../packages/release/src/domain/Dashboard/ReleaseParser.js'
...
/**
 * @typedef {import('@nan0web/release/types/domain/Dashboard/DashboardModel').DashboardSummary} DashboardSummary
...
export class DashboardModel extends ModelAsApp {
	...
	/**
	 * Shortcut to access static UI with proper typing.
	 * @returns {object}
	 */
	get $UI() {
		/** @type {ModelAsApp} */
	 ...
	 */
	async *run() {
		yield progress(this.$UI.aggregating)
		for await (const p of this.aggregateStream()) {
			yield progress(`${p.project} (${p.current}/${p.total})`)
		}
		const t = this._?.t
		const doneMsg = t
			? t(this.$UI.done, { total: this.projects.length })
			: this.$UI.done.replace('{$total}', String(this.projects.length))
	...
	async *aggregateStream() {
		...
		for (const proj of registeredProjects) {
			...
			if (discovered.length === 0) {
				const gitInfo = gitTelem?.getGitInfo
					? gitTelem.getGitInfo(proj.path)
					: { branch: 'main', gitStatus: 'clean' }
