 ...
 * @extends {Message}
 */
class Coverage extends Message {
...
export default class CoverageCommand extends CLI {
	...
	async *run(msg = new Coverage()) {
		...
		yield new OutputMessage(
			`🔍 Mode: ${isSelfPackage ? 'Direct Node Coverage' : 'Spawned Test Runner'}\n`,
		)
		...
		if (isSelfPackage) {
			...
			if (!scripts['test:coverage'] && !scripts.test) {
				throw new Error('No "test:coverage" or "test" script found in package.json')
			}
