...
describe('MemoryDB', () => {
	...
	it('Predefined test data should be loaded during connection', async () => {
		const db = new MemoryDB({
			predefined: [['test.txt', 'test content']],
		})
	...
	it('Path resolution with support for relative and absolute paths', () => {
		const db = new MemoryDB({ root: '/root/' })
		assert.strictEqual(db.relative('/root/dir/file.txt'), 'dir/file.txt')
		assert.strictEqual(db.relative('dir/file.txt'), 'dir/file.txt')
	})
