...
export default class TestPackage {
	...
	static SCRIPTS = {
		...
		'test:coverage':
			'node --experimental-test-coverage --test-coverage-include="src/**/*.js" --test-coverage-exclude="src/**/*.test.js" --test "src/**/*.test.js"',
	...
	static NPM_FILES = ['src/', 'types/', 'package.json', 'README.md']
	 ...
	 */
	constructor(input) {
		const { cwd, db = new DB(), name = '', hash = '', baseURL = '' } = input
		this.cwd = String(cwd || '.')
		this.#db = db
	...
	toMarkdown(rrs) {
		...
		rrs.todo.forEach((name) => {
			md.add(
				new MDHeading2({
					content: name.startsWith('!') ? 'Required ' + name.slice(1) : 'Optional ' + name,
				}),
			)
