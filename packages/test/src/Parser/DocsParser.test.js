...
const fs = new FS({ root: import.meta.dirname })
...
function testRender() {
	...
	it('tDefault is just for example, usually there is no need to use it', () => {
		...
		assert.deepStrictEqual(
			console.output(),
			['Welcome, Anna!', 'Вітаємо у пісочниці, Іван!', 'NonExistingKey'].map((el) => ['info', el]),
		)
	})
	/**
	 * @docs
	...
	it('package.json scripts contain required commands', () => {
		...
		assert.ok(scripts['pre-commit'] || scripts.precommit, 'Missing "pre-commit" script')
	})
}
...
describe('DocsParser', () => {
	...
	it('should parse a function', async () => {
		...
		assert.ok(node)
		assert.deepStrictEqual(String(node).split('\n'), md.split('\n'))
	})
	it('should parse a raw string with top-level comment', () => {
		const parser = new DocsParser()
		const raw = '/**\n * @docs\n * # Title\n */'
		const node = parser.decode(raw)
		assert.ok(node)
		assert.strictEqual(String(node), '# Title\n')
	})
})
