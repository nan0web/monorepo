		...
		const dataset = md.document
			.map((el, i, arr) => {
				if (
					...
					el.content.startsWith('How ') &&
					el.content.includes('?')
				) {
					const next = arr[i + 1]
					if (next instanceof MDCodeBlock) {
						/** @type {MDHeading[]} */
						const contextElements = arr
							.slice(0, i)
							.filter((el) => el instanceof MDHeading)
							.map((el) => MDHeading.from(el))
						const contextMap = new Map()
						/** @type {MDHeading | null} */
						const recentEl = contextElements[contextElements.length - 1]
						const recent = MDHeading.from(recentEl ?? { mdTag: '###### ' })
						contextElements
							.filter((el) => el.heading <= recent.heading)
							.map((el) => contextMap.set(el.tag, `h${el.heading}:${el.content}`))
						const context = Array.from(contextMap.values())
						let contextStart =
							contextElements.length > 0 ? contextElements[contextElements.length - 1] : 0
						...
						const input = arr
							...
							.filter(
								(el) =>
									!(
										el instanceof MDCodeBlock ||
										(el instanceof MDParagraph && el.content.startsWith('How '))
									),
							)
							.map(String)
							.join('\n')
