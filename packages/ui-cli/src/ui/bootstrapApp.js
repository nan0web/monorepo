 ...
 * @property {string} [system='.nan0web'] System directory, used for ~/ (Home dir).
 */
...
async function readMounts(db, { argv, appCwd, pkg, dbConsole, isTestMode, config }) {
	...
	if (!db.mounts?.has('@system')) {
		const sysHome = isTestMode
			...
			: path.join(os.homedir(), config.system || '.nan0web')
		db.mount(
			'@system',
...
export async function bootstrapApp(AppModel, config = {}) {
	...
	const lang =
		langStr ||
		(process.env.LANG ? process.env.LANG.split('_')[0].split('-')[0] : isTestMode ? 'en' : 'uk')
	...
	try {
		...
		const exitCode =
			res.success && !res.cancelled && status !== 'error' && status !== 'failed' ? 0 : 1
