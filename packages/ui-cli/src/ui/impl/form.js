		...
		fields.push(
			new FormInput({
				...
				options: Array.isArray(schema.options)
					? schema.options.map((opt, index) => {
							if (typeof opt === 'string') return opt
							const value = opt.value !== undefined ? opt.value : (opt.name || index)
							return {
								label: translate(opt.UI?.title || opt.label || opt.title || opt.name || opt.value),
								value: value,
							}
					})
					: [],
					...
					validation: f.validation
						? (val) => {
								try {
									...
									if (res === false) return `${this.t(FormModel.UI_VALIDATE_ERROR)} ${f.name}`
									return typeof res === 'string'
										? this.t(res)
										: `${this.t(FormModel.UI_VALIDATE_ERROR)} ${f.name}`
								} catch (error) {
									const err = /** @type {Error} */ (error)
									return err.message || `${this.t(FormModel.UI_VALIDATE_ERROR)} ${f.name}`
								}
							}
						: () => true,
					schema: f,
		...
		for (const [name, schema] of Object.entries(Class)) {
			...
			const validation = validateFn
				? (value) => {
						try {
							...
							if (res === true) return true
							if (res === false) return `${this.t(FormModel.UI_VALIDATE_ERROR)} ${name}`
							if (typeof res === 'string') return this.t(res)
							return `${this.t(FormModel.UI_VALIDATE_ERROR)} ${name}`
						} catch (error) {
							const err = /** @type {Error} */ (error)
							return err.message || `${this.t(FormModel.UI_VALIDATE_ERROR)} ${name}`
						}
					}
				: () => true
			...
			fields.push({
				...
				options: options.map((opt, index) => {
					if (typeof opt === 'string') return { label: this.t(opt), value: opt }
					const label = opt.label || opt.title || opt.name || opt.value || ''
					const value = opt.value !== undefined ? opt.value : (opt.name || index)
					return { ...opt, label: this.t(label), value }
				}),
				validation,
				schema,
				item: schema.item,
			...
			try {
				...
				if (field.type === 'select' || field.options.length > 0) {
					const selConfig = {
						title: field.label,
						message: field.label,
						prompt: `${this.t(FormModel.UI_SELECT)}: `,
					...
					while (!done && retries <= MAX_RETRIES) {
						...
						options.push({ label: `+ ${this.t(FormModel.UI_ADD)}`, value: 'add' })
						options.push({ label: `✔ ${this.t(FormModel.UI_DONE)}`, value: 'done' })

						const sel = await selectFn({
							title: promptMsg,
							message: `${promptMsg} (${arr.length} items)`,
							prompt: this.t(FormModel.UI_ACTION),
							options,
							t: this.t,
							console: this.options.console,
						})
						...
						if (sel.value === 'done') {
							...
							if (typeof template === 'object') {
								...
								const inputObj = await this.input(`${this.t(FormModel.UI_VALUE)}: `)
							...
							const actionSel = await selectFn({
								...
								options: [
									{ label: this.t(FormModel.UI_EDIT), value: 'edit' },
									{ label: this.t(FormModel.UI_DELETE), value: 'delete' },
									{ label: this.t(FormModel.UI_BACK), value: 'cancel' },
								],
								t: this.t,
								console: this.options.console,
							})
								...
								if (actionSel.value === 'delete') {
									...
									if (itemToEdit && typeof itemToEdit === 'object') {
										...
										const inputObj = await this.input(
											`[${idxToEdit}] ${this.t(FormModel.UI_VALUE)} (${itemToEdit}): `
										)
				...
				if (result?.cancelled || (this.options.adapter && this.options.adapter.cancelled)) {
