...
export async function markdownViewer(config) {
	...
	for (let i = 0; i < ast.length; i++) {
		...
		else if (type.includes('FORM') || type === 'FORM' || node.form || node.$id || node.id || node.fields) {
			gutter = 'frm |';
			color = Logger.MAGENTA;
			const formModel = node.form || node
			const formTitle = t(formModel.title || node.$title || node.title || (formModel.constructor && formModel.constructor.name !== 'Object' ? formModel.constructor.name : 'Unnamed'))
			text = `[ Form: ${formTitle} ]`;
			nodeType = 'form'
		}
