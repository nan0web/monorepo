...
export function Table(props) {
	...
	return createPrompt('Table', model, async (p) => {
		...
		return await baseTable({
			...p,
			title: t(p.UI?.default || p.UI),
			filterMsg: t(TableModel.UI_FILTER?.default || TableModel.UI_FILTER),
			noneMsg: t(TableModel.UI_NONE?.default || TableModel.UI_NONE),
			filterPrompt: t(TableModel.UI_FILTER_PROMPT?.default || TableModel.UI_FILTER_PROMPT),
		})
	})
}
