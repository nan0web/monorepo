...
### Added
- **Verified Documentation (ProvenDoc v2)**: Replaced all legacy `console.info` placeholders in `README.md.js` with functional `render()` calls for `Alert`, `Table`, `Spinner`, and `ProgressBar`, ensuring all documentation examples are live-tested and accurate.
- **Generator Documentation**: Added comprehensive section on "One Logic, Many UI (Generators)" demonstrating component rendering via standard OLMUI intents (`yield { type: 'render', ... }`).

### Fixed
- **ProgressBar API Inconsistency**: Corrected `ProgressBar` example in documentation (switched from non-existent `.stop()` to `.success()`).
- **Table Test Stability**: Added `interactive: false` to documentation table examples to prevent test timeouts while maintaining component verification.
- **Documentation Regression Testing**: Introduced a permanent release contract (`task.spec.js`) to enforce documentation integrity and prevent the return of placeholder comments.

## [2.13.1] — 2026-05-05

### Changed
- **Maintenance**: Bumping version to align with documentation stabilization cycle.
