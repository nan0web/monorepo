...
describe('mockFetch', () => {
	...
	it('should handle exact route match', async () => {
		const routes = [['GET /users', { id: 1, name: 'John' }]]
	...
	it('should handle method wildcard', async () => {
		const routes = [['* /users', { message: 'Method wildcard match' }]]
		const fetch = mockFetch(routes)
	...
	it('should handle path wildcard', async () => {
		const routes = [['GET /items/*', { message: 'Path wildcard match' }]]
		const fetch = mockFetch(routes)
	...
	it('should match all with * *', async () => {
		const routes = [['* *', { message: 'Catch all match' }]]
		const fetch = mockFetch(routes)
	...
	it('should handle custom status code', async () => {
		const routes = [['POST /users', [201, { id: 2, name: 'Jane' }]]]
		const fetch = mockFetch(routes)
		const response = await fetch('/users', { method: 'POST' })
		const data = await response.json()
	...
	it('should handle function-based response', async () => {
		const routes = [['GET /dynamic', async () => ({ dynamic: true })]]
		const fetch = mockFetch(routes)
	...
	it('should handle function-based response returning structured response', async () => {
		const routes = [['GET /structured', async () => [418, { message: 'I am a teapot' }]]]
		const fetch = mockFetch(routes)
	...
	it('should handle HEAD responses', async () => {
		const fetch = mockFetch([['HEAD /index.html', { 'Content-Type': 'text/html' }]])
