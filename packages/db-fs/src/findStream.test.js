...
class TestEntry {
	...
	errorsSize
	constructor(input = []) {
		const [file, dirSize = 0, topSize = 0, totalSize = { dirs: 0, files: 0 }, errorsSize = 0] =
			input
...
suite('findStream()', () => {
	...
	const createMemoryFS = () => {
		...
		return {
			...
			statSync: (path) => {
				...
				return {
					mtime: fs.get(path).mtime,
					mtimeMs: fs.get(path).mtimeMs,
					size: fs.get(path).content.length,
				}
			},
			load: (path) => {
				if (!fs.has(path)) throw new Error('File not found')
	...
	beforeEach(() => {
		...
		files = [
			...
			new DocumentEntry({
				name: 'dir',
				stat: new DocumentStat({ size: 0, mtimeMs: 3000 }),
				depth: 0,
				isDirectory: true,
			}),
			new DocumentEntry({
	...
	it('should yield files with correct progress and sorting by name asc', async () => {
		let entries = [
			new DocumentEntry({
				path: 'a.txt',
				stat: new DocumentStat({ size: 10, mtimeMs: 1000, isFile: true }),
				depth: 0,
			}),
			new DocumentEntry({
				path: 'b.txt',
				stat: new DocumentStat({ size: 20, mtimeMs: 2000, isFile: true }),
				depth: 0,
			}),
			new DocumentEntry({
				path: 'c.txt',
				stat: new DocumentStat({ size: 30, mtimeMs: 3000, isFile: true }),
				depth: 0,
			}),
			new DocumentEntry({
				path: 'dir',
				stat: new DocumentStat({ size: 30, mtimeMs: 3000, isDirectory: true }),
				depth: 0,
			}),
			new DocumentEntry({
				path: 'dir/file3.txt',
				stat: new DocumentStat({ size: 30, mtimeMs: 4000, isFile: true }),
				depth: 1,
			}),
			new DocumentEntry({
				path: 'dir/inc',
				stat: new DocumentStat({ size: 30, mtimeMs: 4000, isDirectory: true }),
				depth: 1,
			}),
			new DocumentEntry({
				path: 'dir/inc/index.js',
				stat: new DocumentStat({ size: 30, mtimeMs: 4000, isFile: true }),
				depth: 2,
			}),
			new DocumentEntry({
	...
	it('should respect limit option', async () => {
		const entries = [
			new DocumentEntry({
				name: 'a.txt',
				stat: new DocumentStat({ size: 10, mtime: new Date(1000), mtimeMs: 1000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'b.txt',
				stat: new DocumentStat({ size: 20, mtime: new Date(2000), mtimeMs: 2000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'c.txt',
				stat: new DocumentStat({ size: 30, mtime: new Date(3000), mtimeMs: 3000 }),
				depth: 0,
			}),
		]
		db.readDir = async function* () {
	...
	it('should sort by mtime desc', async () => {
		const entries = [
			...
			new DocumentEntry({
				name: 'b.txt',
				stat: new DocumentStat({ size: 20, mtime: new Date(3000), mtimeMs: 3000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'c.txt',
				stat: new DocumentStat({ size: 30, mtime: new Date(2000), mtimeMs: 2000 }),
				depth: 0,
			}),
		]
		db.readDir = async function* () {
	...
	it('should sort by size asc', async () => {
		const entries = [
			new DocumentEntry({
				name: 'a.txt',
				stat: new DocumentStat({ size: 30, mtime: new Date(1000), mtimeMs: 1000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'b.txt',
				stat: new DocumentStat({ size: 10, mtime: new Date(3000), mtimeMs: 3000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'c.txt',
	...
	it('should handle errors in stat and collect them', async () => {
		...
		const entries = [
			...
			new DocumentEntry({
				name: 'b.txt',
				path: 'b.txt',
				stat: new DocumentStat({ size: 20, mtime: new Date(2000), mtimeMs: 2000, error }),
				depth: 0,
			}),
		]
		db.readDir = async function* () {
	...
	it('should correctly handle findStream root directory scan', async () => {
		const entries = [
			new DocumentEntry({
				name: '.',
				path: '.',
				stat: new DocumentStat({ isDirectory: true }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'file1.txt',
	...
	it('should handle nested directories correctly with findStream', async () => {
		const entries = [
			...
			new DocumentEntry({
				name: 'dir/nested/deep.txt',
				path: 'dir/nested/deep.txt',
				stat: new DocumentStat({ size: 50, mtimeMs: 5000 }),
				depth: 2,
			}),
			new DocumentEntry({
				name: 'dir/file.txt',
				path: 'dir/file.txt',
				stat: new DocumentStat({ size: 30, mtimeMs: 3000 }),
				depth: 1,
			}),
		]
		db.readDir = async function* () {
	...
	it('should handle mixed content types in findStream', async () => {
		const entries = [
			new DocumentEntry({
				name: 'data.json',
				path: 'data.json',
				stat: new DocumentStat({ size: 100, mtimeMs: 1000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'style.css',
				path: 'style.css',
				stat: new DocumentStat({ size: 200, mtimeMs: 2000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'index.html',
				path: 'index.html',
				stat: new DocumentStat({ size: 300, mtimeMs: 3000 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'script.js',
				path: 'script.js',
				stat: new DocumentStat({ size: 150, mtimeMs: 1500 }),
				depth: 0,
			}),
			new DocumentEntry({
				name: 'dir',
			...
			new DocumentEntry({
				name: 'dir/image.png',
				path: 'dir/image.png',
				stat: new DocumentStat({ size: 500, mtimeMs: 5000 }),
				depth: 1,
			}),
		]
		db.readDir = async function* () {
