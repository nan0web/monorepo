...
describe('toSvg', () => {
	...
	it('renders IbLogo with custom size', () => {
		const svg = toSvg(IbLogo, { size: 32 })
...
describe('icon data structure', () => {
	...
	it('IbLogo has correct shape', () => {
		assert.equal(IbLogo.tag, 'svg')
		assert.ok(IbLogo.attr.viewBox)
		assert.ok(Array.isArray(IbLogo.child))
	})
})
