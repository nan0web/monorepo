...
## 📥 УНІФІКОВАНИЙ ФОРМАТ ПОВЕРНЕННЯ РЕЗУЛЬТАТІВ (Unified Result Contract)

Кожна Доменна Модель або Аудитор, що завершує свою роботу, повертає фінальну інтенцію результату через хелпер `result(data)`. Для забезпечення узгодженості та надійності пайплайнів NaN0Web, формат повернення результатів стандартизується з використанням таких обов'язкових полів:

1.  **`ok: boolean`**: Ключовий прапорець успішності виконання (аналогічно нативному `Response.ok`). Повністю замінює legacy-поле `success`.
2.  **`code: number`**: Стандартний HTTP-подібний код статусу:
    -   `200` — Успішне виконання без зауважень.
    -   `400` — Помилка верифікації (невідповідність бізнес-правилам, лінт-помилки).
    -   `404` — Цільові об'єкти чи файли не знайдені.
    -   `500` — Критичний системний збій (відсутність БД, аварійний вихід).
3.  **Зворотна сумісність**: Усі інструменти та базові моделі-оркестратори (наприклад, `ArchitectureAuditor`) зобов'язані перевіряти результат, підтримуючи як сучасний параметр `ok`, так і legacy-параметр `success` у якості фолбека:
    ```javascript
