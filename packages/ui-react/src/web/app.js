...
async function bootstrap(forcedLocale = null) {
	...
	localStorage.setItem('nan0ui-locale', locale)

	const themeChoice = localStorage.getItem('nan0ui-theme') || 'auto'
	applyTheme(themeChoice)

	// Load YAML data
	const res = await fetch(`${import.meta.env.BASE_URL}docs/data/${locale}/index.yaml?t=${Date.now()}`)
	if (!res.ok) {
		console.error(`Failed to load docs/data/${locale}/index.yaml`)
		return
	}
	...
	document.title = data.title || 'NaN•UI React'
	...
	if (!isInitialized) {
		...
		window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
			const tc = localStorage.getItem('nan0ui-theme') || 'auto'
