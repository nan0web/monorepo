...
describe('OutputMessage', () => {
	...
	it('should create instance with custom values', () => {
		...
		assert.ok(msg.content[0].startsWith('Error: '))
		assert.deepEqual(msg.content[1], 'test content')
		assert.deepEqual(msg.head, { key: 'value' })
	...
	it('should set type based on error presence', () => {
		const errorMsg = new OutputMessage(new Error('error occurred'))
